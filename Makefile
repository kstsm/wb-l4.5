...
bench-compare:
	benchstat $(BENCH_DIR)/before.txt $(BENCH_DIR)/after.txt
...
bench-cpu:
	mkdir -p benchmarks
	hey -z 60s -c 100 -m POST -T "application/json" \
	-d '{"items":["alpha","beta","gamma","delta","epsilon","zeta","eta","theta","iota","kappa"]}' \
	http://localhost:8080/concatenate & \
	sleep 5 && \
	curl -o benchmarks/cpu.prof \
	http://localhost:6060/debug/pprof/profile?seconds=30

# Heap профиль под нагрузкой
bench-heap:
	mkdir -p benchmarks
	hey -z 60s -c 100 -m POST -T "application/json" \
	-d '{"items":["alpha","beta","gamma","delta","epsilon","zeta","eta","theta","iota","kappa"]}' \
	http://localhost:8080/concatenate & \
	sleep 30 && \
	curl -o benchmarks/heap.prof \
	http://localhost:6060/debug/pprof/heap


# go tool pprof benchmarks/heap.prof